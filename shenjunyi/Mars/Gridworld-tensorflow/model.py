...
def VI_Block(X, S1, S2, config):
    ...
    w_transfer = tf.Variable(tf.ones([ch_q]), dtype=tf.float32, name="w_transfer")
# 4 actions:
# w_o_new = tf.Variable(np.random.randn(ch_q, 4) * 0.01, dtype=tf.float32, name='w_o_new')
